...
export function notificationConfig(env) {
  const recipients = [...new Set((env.ANDROID_TESTER_EMAILS || '')
  ...
  if (url.origin !== 'https://play.google.com' ||
      url.pathname !== '/apps/testing/live.logmaster.app' || url.search || url.hash) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(() => {
    console.error('Notification failed: check required configuration and SES access. No raw credentials or provider errors are logged.')
    process.exitCode = 1
  })
}
