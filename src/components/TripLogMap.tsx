...
import {
  ...
  buildLegTrackGeoJsonWithWaypointDraft,
  mapBrandColor,
  mapPointsToBounds,
...
const TripLogMapMapLibre = forwardRef<TripMapHandle, TripLogMapProps>(
  function TripLogMapMapLibreView(
    {
      trip,
      entries,
      legs = [],
    ...
    const pickCenterPosition = useMapCenterPosition(
      mapRef,
      mapReady,
      waypointCenterPickActive,
    )
    ...
    const hoveredEntryRecord = hoveredEntry
      ? (entriesById.get(hoveredEntry.entryId) ?? null)
      : null
    ...
    useEffect(() => {
      ...
      if (!marker) {
        marker = new maplibregl.Marker({
          element: createBoatMapMarkerElementForIconId(
            boatIconId,
    ...
    useEffect(() => {
      ...
      const entry = entries.find(
        (item) => item.id === waypointPick.editingEntryId,
      )
      if (
        entry?.latitude == null ||
            ...
            <SailingMapControlStack
              ...
              layers={
                <SailingMapLayerPanel
                  ...
                  aisSavedTripHint={
                    isSavedTrip && !(playbackMode && playbackPlaying)
                  }
                />
              }
              onExpand={
        ...
        {waypointCenterPickActive && waypointPick ? (
          <>
            ...
            <WaypointCenterPickOverlay
              ...
              confirmLabel={
                waypointPick.phase === 'edit-pick'
                  ? 'Save waypoint position'
                  : 'Add waypoint'
              }
              hideCenterMarker={waypointPick.phase === 'edit-pick'}
              onDelete={
                waypointPick.phase === 'edit-pick'
                  ? waypointPick.onDelete
                  : undefined
              }
              onConfirm={() => {
