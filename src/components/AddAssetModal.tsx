...
import { equipmentDocuments, hasLocalizedDocuments, languageRank, productModelKey } from "../domain/product-catalog";
...
export type ListedBoatAsset = Pick<BoatAsset, "id" | "name" | "description" | "modelNumber" | "category"> & {
  brand?: string | null;
};
...
export function findExistingBoatAsset(
  ...
  const candidates = assets.filter((asset) => {
    ...
    return !identity.brand || !brand || normalized(identity.brand) === normalized(brand);
  });
  ...
  return identity.productName
    ? (candidates.find(
        (asset) => normalized(getAssetIdentity(asset).productName) === normalized(identity.productName),
      ) ?? null)
    : null;
}
type Step =
...
export function AddAssetModal({
  ...
  const [busy, setBusy] = useState<"camera" | "identify" | "lookup" | "connections" | "save" | null>(null);
  ...
  const [connectionPeers, setConnectionPeers] = useState<ConnectionPeerOption | null>(null);
  ...
  const [suggestedModels, setSuggestedModels] = useState<EquipmentModelOption[]>([]);
  ...
  const stage = step === "find" || step === "results" ? 0 : step.startsWith("document") ? 1 : 2;
  ...
  function connectionName(connection: Pick<AssetConnectionSuggestion, "assetId" | "name">) {
    return (
      connection.name ||
      assets.find((asset) => asset.id === connection.assetId)?.name ||
      networkCandidates.find((network) => network.id === connection.assetId)?.name ||
      connection.assetId
    );
  }
  const documents = equipmentDocuments(product, language);
  const privateDocuments = product ? [] : (research?.downloads.filter((item) => item.purpose !== "photo") ?? []);
  ...
  useEffect(() => {
    if (step !== "connectionPrompt" && step !== "connections" && step !== "addConnection")
      return;
    if (connectionPeers) return;
    const controller = new AbortController();
    void fetchConnectionPeers(boatId)
      .then((peers) => {
        if (!controller.signal.aborted && mounted.current) setConnectionPeers(peers);
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const job = await fetchAssetResearchJob(boatId, jobId!, controller.signal);
        if (controller.signal.aborted) return;
        if (job.status === "completed" && job.result) {
          setResearch(job.result);
          if (job.result.productId) {
            const updated = await fetchCatalogProduct(job.result.productId, language, controller.signal);
            if (controller.signal.aborted) return;
            setProduct(updated);
          }
          setResearchActive(false);
          setResearchError("");
          setStep((current) => (current === "documentProgress" ? "documents" : current));
          return;
        }
        ...
        if (controller.signal.aborted) return;
        setResearchError(error instanceof Error ? error.message : t("addAssetSuggestionsUnavailable"));
  ...
  function close() {
    ...
    if (!(brand || model || description || photo || productLink.trim()) || window.confirm(t("equipmentCloseConfirm"))) {
      onClose();
    }
  }
  ...
  function canSearch() {
    ...
    return !noModel ? brand.trim().length > 0 && model.trim().length > 0 : description.trim().length > 0;
  }
  ...
  async function identifyFromPhoto() {
    ...
    try {
      ...
      if (!controller.signal.aborted && mounted.current)
        setNotice(error instanceof Error ? error.message : t("addAssetIdentifyFailedFallback"));
      return null;
    } finally {
  ...
  async function openCamera() {
    ...
    try {
      ...
      if (mounted.current) {
        setBusy(null);
        if (!/cancel/i.test(String(error))) setNotice(t("addAssetCameraUnavailable"));
      }
    }
  }
  ...
  async function lookupCatalogProduct(catalog?: {
    ...
    try {
      ...
      let result = await resolveEquipmentProduct(brandValue, modelValue, language, controller.signal, hints);
      while (result.pending && !controller.signal.aborted) {
        ...
        if (controller.signal.aborted) return;
        result = await resolveEquipmentProduct(brandValue, modelValue, language, controller.signal, hints);
      }
      ...
      if (!controller.signal.aborted && mounted.current) {
        ...
        setNotice(error instanceof Error ? error.message : t("productLookupUnavailable"));
      }
    } finally {
      if (!controller.signal.aborted && mounted.current) setBusy(null);
    }
  ...
  async function searchCatalogByModel(typedBrand: string, typedModel: string) {
    ...
    setProduct(null);
    try {
      const match = await findCatalogProducts(typedBrand, typedModel, language, controller.signal);
      if (controller.signal.aborted || !mounted.current) return;
      ...
      const suggested = await fetchEquipmentModelOptions(typedBrand, typedModel, controller.signal);
      if (controller.signal.aborted || !mounted.current) return;
      const catalog = suggested.products ?? [];
      if (suggested.brandCorrect && catalog.length === 1 && !suggested.ambiguous) {
        await selectCatalogProduct(catalog[0]);
        return;
      }
      if (catalog.length || suggested.brandGuesses.length) {
        showModelPicker(catalog, [], suggested.brandGuesses, suggested.brandCorrect);
        return;
      }
      if (suggested.brandCorrect && suggested.options.length === 1 && !suggested.ambiguous) {
        await selectSuggestedModel(suggested.options[0]);
        return;
      }
      if (suggested.options.length || suggested.brandGuesses.length) {
        showModelPicker([], suggested.options, suggested.brandGuesses, suggested.brandCorrect);
        return;
      }
      setLookupFailed(true);
  ...
  async function runSearch() {
    ...
    const identified = findMode === "link" ? await identifyFromLink() : await identifyFromPhoto();
  ...
  function startDocumentsStep() {
    setNotice("");
    setStep(
      researchActive ? "documentProgress" : hasLocalizedDocuments(product, language) ? "documents" : "documentPrompt",
    );
  }
  ...
  async function searchConnections() {
    ...
    try {
      ...
      if (!controller.signal.aborted && mounted.current)
        setNotice(error instanceof Error ? error.message : t("addAssetSuggestionsUnavailable"));
    } finally {
      if (!controller.signal.aborted && mounted.current) setBusy(null);
    }
  }
    ...
    try {
      const asset = await createBoatAsset(
        boatId,
        {
          ...
          installedAt: installedAt ? new Date(`${installedAt}T12:00:00`).toISOString() : null,
          researchJobId: jobId ?? undefined,
          suggestedDownloads: product ? [] : (research?.downloads ?? []),
          confirmedConnections: connections.filter((item) => selected.includes(item.assetId)),
        },
        photo,
      );
      onCreated(asset);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : t("addAssetSaveFailed"));
  ...
  async function mergeExisting() {
    ...
    try {
      ...
      const updated = await updateBoatAsset(boatId, existing.id, {
        ...
        description:
          current && incoming && current.toLowerCase() !== incoming.toLowerCase()
            ? `${current}\n\n${incoming}`
            : current || incoming || null,
      });
      if (photo) await uploadAndLinkAssetDocument(boatId, existing.id, photo, "photo");
      onUpdated(updated);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : t("addAssetUpdateExistingFailed"));
      saving.current = false;
      setBusy(null);
    }
  }
  const action = (label: string, onClick: () => void, disabled = false, next = false) => (
    <button type="button" className={primary} disabled={disabled} onClick={onClick}>
      {label}
      {next && <ArrowRight className="size-4" />}
    </button>
  );
  const back = (onClick: () => void) => (
    <button type="button" className={secondary} onClick={onClick} disabled={busy === "save"}>
  ...
  if (step === "find")
    footer = (
      <>
        {findMode === "model" &&
          noModel &&
          action(t("equipmentNext"), continueWithoutSearch, !!busy || !description.trim(), true)}
        {action(t("equipmentSearch"), () => void runSearch(), !!busy || !canSearch())}
      </>
    );
    ...
    footer = (
      <>
        <button type="button" className={secondary} onClick={() => setStep("documents")} disabled={researchStarting}>
          {t("equipmentSkip")}
        </button>
        {action(researchStarting ? t("loading") : t("equipmentOkay"), () => void startDocuments(), researchStarting)}
      </>
    );
    ...
    footer = (
      <>
        <button type="button" className={secondary} onClick={() => setStep("connections")}>
          {t("equipmentSkip")}
        </button>
    ...
    footer = (
      <>
        ...
        {action(
          ...
          () => {
            if (!manualId) return;
            const network = networkCandidates.find((item) => item.id === manualId);
  ...
  function intro(title: string, help: string) {
    return (
      <div className="mb-6">
        ...
        <p className="mb-0 mt-2 text-sm leading-relaxed text-[var(--sea-ink-soft)]">{help}</p>
      </div>
    );
  }
  function loading(title: string, help?: string) {
    return (
      <div role="status" className="flex min-h-64 flex-col items-center justify-center gap-5 py-10 text-center">
        ...
        <div>
          ...
          {help && <p className="mb-0 mt-3 max-w-sm text-sm leading-relaxed text-[var(--sea-ink-soft)]">{help}</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    <EquipmentFlowDialog
      title={step === "addConnection" ? t("equipmentAddConnection") : t("equipmentTitle")}
      stage={stage}
      stages={[t("equipmentStage"), t("equipmentDocuments"), t("equipmentConnections")]}
      stepKey={step}
      closeLabel={t("close")}
      suspended={!!viewer}
      onClose={close}
    ...
    >
      {step === "find" && (
        <>
          ...
          >
            {(
              [
                { id: "photo" as const, label: t("equipmentFindModePhoto"), icon: CameraIcon },
                { id: "model" as const, label: t("equipmentFindModeModel"), icon: Hash },
                { id: "link" as const, label: t("equipmentFindModeLink"), icon: Link2 },
              ] as const
            ).map((mode) => (
              <button
                key={mode.id}
                ...
                className={`flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-full px-2 text-sm font-semibold transition-colors disabled:opacity-50 ${
                  findMode === mode.id ? "bg-[var(--btn-bg)] text-[var(--btn-text)]" : "text-[var(--sea-ink-soft)]"
                }`}
                onClick={() => {
          ...
          {findMode === "photo" && (
            <div className="relative mb-2 flex min-h-36 flex-col items-center justify-center py-4">
              ...
              >
                {preview ? (
                  <img src={preview} alt={t("addAssetPhotoAlt")} className="absolute inset-0 size-full object-cover" />
                ) : (
              ...
              <p className="mb-0 mt-3 text-xs text-[var(--sea-ink-soft)]">{t("equipmentCameraHint")}</p>
              <input
                ref={input}
                type="file"
                accept="image/*"
          ...
          <>
            ...
            <div className="mb-5 space-y-4">
              ...
              <button
                ...
                onClick={() => void searchCatalogByModel(brand.trim(), model.trim())}
              >
            ...
            />
            {!catalogMatches.length && !suggestedModels.length && !brandGuesses.length && (
              <p className="text-sm text-[var(--sea-ink-soft)]">{t("equipmentPickModelRequired")}</p>
            )}
          </>
        ) : (
          <>
              ...
              <ul className="mb-4 list-none space-y-1 rounded-2xl bg-[var(--chip-bg)] p-4 text-sm text-[var(--sea-ink-soft)]">
                {product.info.specifications.slice(0, 6).map((spec) => (
                  <li key={`${spec.name}-${spec.value}`}>
                    <span className="font-semibold text-[var(--sea-ink)]">{spec.name}:</span> {spec.value}
                    {spec.unit ? ` ${spec.unit}` : ""}
                  </li>
                ))}
              </ul>
            ...
            <h3 className="mb-1 mt-3 break-words text-3xl font-bold tracking-tight">{model || name || description}</h3>
            {name && model && <p className="mt-1 text-sm text-[var(--sea-ink-soft)]">{name}</p>}
            {product?.createdAt && (
              <p className="text-xs text-[var(--sea-ink-soft)]">
                {t("equipmentAdded", {
                  date: new Date(product.createdAt).toLocaleDateString(language),
                })}
              </p>
            )}
            {lookupFailed && (
              <button type="button" className={`${secondary} my-3`} onClick={() => void lookupCatalogProduct()}>
                {t("equipmentRetry")}
              </button>
            )}
            ...
            <details className="mt-5 border-t border-[var(--line)] pt-4">
              <summary className="cursor-pointer py-2 text-sm font-semibold">{t("equipmentDetails")}</summary>
              <div className="mt-3 space-y-4">
                ...
                <label className="block space-y-2 text-sm">
                  ...
                  <select
                    className={field}
                    value={ownership}
                    onChange={(event) => setOwnership(event.target.value as AssetOwnership)}
                  >
            ...
            {existing && (
              <div className="mt-5 rounded-2xl bg-[var(--chip-bg)] p-4">
                <p className="mt-0 text-sm leading-relaxed">{t("equipmentExistingHelp")}</p>
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    className={secondary}
                  ...
                  <button type="button" className={secondary} disabled={!!busy} onClick={() => void mergeExisting()}>
                    {t("merge")}
                  </button>
                  <button type="button" className={secondary} disabled={!!busy} onClick={() => setAnotherUnit(true)}>
      ...
      {step === "documentProgress" && (
        <>
          {researchActive
            ? loading(t("equipmentDocumentsProgress"), t("equipmentDocumentsProgressHelp"))
            : intro(t("equipmentDocumentsReady"), t("equipmentDocumentsEmpty"))}
          {researchError && !researchActive && (
            <button type="button" className={secondary} onClick={() => void startDocuments()}>
              {t("equipmentRetry")}
            </button>
          )}
        </>
      ...
      {step === "documents" && (
        <>
          {intro(t("equipmentDocumentsReady"), t("equipmentDocumentsReadyHelp"))}
          {researchActive && (
            <p role="status" className="mb-5 flex items-center gap-2 text-sm text-[var(--sea-ink-soft)]">
              <LoaderCircle className="size-4 animate-spin" />
              {t("equipmentDocumentsBackground")}
            </p>
          )}
          {documents.length > 0 && languageRank(documents[0].languages, language) === 3 && (
            <p className="text-sm text-[var(--sea-ink-soft)]">{t("equipmentEnglishFallback")}</p>
          )}
      ...
      {step === "connectionPrompt" && (
        <div className="py-7">
          ...
          {intro(t("equipmentConnectionsPrompt"), t("equipmentConnectionsHelp"))}
          {connectionCandidates.length > 0 && (
            <span className="rounded-full bg-[var(--chip-bg)] px-3 py-2 text-xs font-semibold">
              {t("equipmentConnectionBoatAssets", { count: connectionCandidates.length })}
            </span>
          )}
          <button
            type="button"
      ...
      {step === "connections" &&
        (busy === "connections" ? (
          ...
          <>
            {intro(t("equipmentConnectionsReview"), t("equipmentConnectionsReviewHelp"))}
            {connections.length ? (
              <div className="space-y-3">
                {connections.map((connection) => (
                  ...
                  >
                    <input
                      ...
                      disabled={!!busy || (!selected.includes(connection.assetId) && selected.length >= 20)}
                      onChange={(event) =>
                        setSelected(
                          event.target.checked
                            ? [...selected, connection.assetId]
                            : selected.filter((id) => id !== connection.assetId),
                        )
                      }
                    />
            ...
            {researchActive && (
              <p className="mt-5 text-xs text-[var(--sea-ink-soft)]">{t("equipmentDocumentsBackground")}</p>
            )}
          </>
        ))}
      {step === "addConnection" && (
        <>
          {intro(t("equipmentChooseConnection"), t("equipmentChooseConnectionHelp"))}
          <div className="space-y-3">
            {networkCandidates.filter((network) => !selected.includes(network.id)).length > 0 && (
            ...
            {connectionCandidates.filter((asset) => !selected.includes(asset.id)).length > 0 && (
              <p className="text-xs font-semibold uppercase tracking-wide text-[var(--sea-ink-soft)]">
          ...
          {!connectionCandidates.some((asset) => !selected.includes(asset.id)) &&
            !networkCandidates.some((network) => !selected.includes(network.id)) && (
            <p className="text-sm text-[var(--sea-ink-soft)]">{t("equipmentNoConnectionCandidates")}</p>
          )}
        </>
      )}
      {researchError && stage === 1 && (
        <p role="alert" className="mt-5 rounded-xl bg-[var(--chip-bg)] p-4 text-sm">
          {researchError}
        </p>
      )}
      {notice && (
        <p role="status" className="mt-5 rounded-xl bg-[var(--chip-bg)] p-4 text-sm leading-relaxed">
          {notice}
        </p>
      )}
      {viewer && <BoatDocumentViewerModal {...viewer} onClose={() => setViewer(null)} />}
    </EquipmentFlowDialog>
  );
}
