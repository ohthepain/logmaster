...
beforeEach(() => {
  ...
  mocks.brands.mockResolvedValue([{ name: "Garmin", logo: "/brands/garmin.svg" }]);
  ...
  mocks.connections.mockResolvedValue([{ assetId: "gps", reason: "Position data" }]);
...
it("uses the catalog automatically and settles brand/model into editable identity labels", async () => {
  ...
  click("Back");
  expect(screen.getByRole("button", { name: "Edit Model" }).textContent).toContain("923");
...
it("keeps the photo attached and identifies when the user searches", async () => {
  ...
  click("Add equipment");
  await waitFor(() =>
    expect(mocks.create).toHaveBeenCalledWith("boat", expect.objectContaining({ modelNumber: "923" }), photo),
  );
});
it("opens the OS chooser for a camera or existing photo on native devices", async () => {
  ...
  expect(mocks.camera).toHaveBeenCalledWith(expect.objectContaining({ source: "PROMPT", saveToGallery: false }));
});
...
it("lets document research continue in the background and links the job on save", async () => {
  ...
  click("Add equipment");
  await waitFor(() =>
    expect(mocks.create).toHaveBeenCalledWith("boat", expect.objectContaining({ researchJobId: "job" }), undefined),
  );
});
...
it("ignores a stale model lookup after going back and changing the model", async () => {
  ...
  expect(screen.getByRole("button", { name: "Edit Model" }).textContent).toContain("943");
...
it("keeps confirmed choices after a save failure and prevents duplicate submissions", async () => {
  ...
  expect(screen.getByRole<HTMLInputElement>("checkbox", { name: /GPS receiver/ }).checked).toBe(true);
  click("Add equipment");
...
it("lets the user pick or correct search terms when AI model lookup is ambiguous", async () => {
  ...
  mocks.models.mockResolvedValue({ products: [], exact: false, ambiguous: false });
  ...
  click("Search");
  await screen.findByText("Choose the exact model");
  expect(document.querySelector('img[src*="/api/products/shu-300/"]')).toBeTruthy();
  expect(screen.getByRole("button", { name: "Next" }).hasAttribute("disabled")).toBe(true);
...
it("offers brand guesses when the typed manufacturer is wrong", async () => {
  mocks.models
    ...
    .mockResolvedValueOnce({ products: [product], exact: true, ambiguous: false });
  mocks.modelOptions.mockResolvedValue({
    brandCorrect: false,
  ...
  enter("Model", "smart shunt 300a ip65");
  click("Search");
  await screen.findByText("This does not look like a Raymarine product. Did you mean one of these brands?");
