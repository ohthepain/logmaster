...
export function FtueOverlay({ onComplete }: FtueOverlayProps) {
  ...
  const stepCount = signedIn ? (skipPwaStep ? 1 : 2) : skipPwaStep ? 2 : 3
        ...
        {!showAuthStep ? (
          <footer className="ios-map-touch-target relative z-10 shrink-0 px-6 pb-[calc(env(safe-area-inset-bottom,0px)+2rem)] pt-2 sm:px-8">
            <button
              type="button"
              onClick={goNext}
              className="ftue-cta ios-map-touch-target touch-manipulation"
            >
...
function FtueFeaturesStep() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-1 flex-col">
      ...
      <h1 className="text-xl rise-in mb-8">
        The all-in-one logging app for your sailing adventures
      </h1>
      ...
      {installed ? (
        ...
        <div className="mt-8">
          <button
            type="button"
            onClick={onInstall}
            className="ftue-secondary-btn ios-map-touch-target touch-manipulation"
          >
          ...
          {!canInstall && (
            <p className="m-0 mt-3 max-w-sm text-sm leading-6 text-[var(--ftue-ink-soft)]">
              On Android or desktop, use your browser&apos;s install option when
              it appears.
            </p>
          )}
        </div>
