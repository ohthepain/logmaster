...
export function BackgroundTripRecorder() {
  ...
  const appendTripTrackPosition = useLogbookStore(
    (state) => state.appendTripTrackPosition,
  )
  const upsertTripTracks = useLogbookStore((state) => state.upsertTripTracks)
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true
      if (activeTripId) {
        void upsertTripTracks(getTripTrackRecorder().sealTrip(activeTripId))
      }
