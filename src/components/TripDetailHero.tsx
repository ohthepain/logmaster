...
export const TripDetailHero = forwardRef<TripMapHandle, TripDetailHeroProps>(
  function TripDetailHeroView(
    ...
    const pauseDevTripRetrip = useAppOptionsStore(
      (state) => state.pauseDevTripRetrip,
    )
    const resumeDevTripRetrip = useAppOptionsStore(
      (state) => state.resumeDevTripRetrip,
    )
    const setDevTripRetripTimescale = useAppOptionsStore(
      (state) => state.setDevTripRetripTimescale,
    )
    const stopDevTripRetrip = useAppOptionsStore(
      (state) => state.stopDevTripRetrip,
    )
    ...
    const showInteractiveMap =
      isActiveTrip || isPlayback || cover.kind === 'map'
    const showPhoto =
      !showInteractiveMap && cover.kind === 'photo' && cover.photoUrl
    ...
    const retripSourceTrip = useMemo(
      () =>
        devTripRetrip
          ? (allTrips.find((item) => item.id === devTripRetrip.sourceTripId) ??
            null)
          : null,
    ...
    const infoEntries =
      devTripRetrip && retripSourceTrip ? retripSourceEntries : mapEntries
    ...
    return (
      ...
      >
        <DevComponentLabel
          name="TripDetailHero"
          className="absolute left-3 top-14 z-20 sm:left-4"
        />
        ...
        {useExternalIosMapControls ? (
          <SailingMapControlStack
            ...
            onExpand={
              showInteractiveMap ? () => setMapFullscreenOpen(true) : undefined
            }
          />
        ) : null}
