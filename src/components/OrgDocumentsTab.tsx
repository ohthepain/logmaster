...
import type { OrgDocument, OrgDocumentCategory } from '../domain/org'
import {
  createOrgDocumentLink,
  createOrgDocumentUpload,
  fetchOrgDocuments,
  fetchOrgLinkDocumentTitle,
...
import { openOrgDocumentRecord } from '../lib/org-document-open'
...
export function OrgDocumentsTab({ orgId }: OrgDocumentsTabProps) {
  ...
  const addCategory = (category: OrgDocumentCategory) => {
  ...
  const handleOpenDocument = async (document: OrgDocument) => {
    try {
  ...
  const handleCreateUpload = async (file: File) => {
    ...
    setBusy(true)
    try {
      const document = await createOrgDocumentUpload(orgId, {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetchOrgLinkDocumentTitle(url)
        .then((pageTitle) => {
  ...
  const handleCreateLink = async () => {
    ...
    setBusy(true)
    try {
      const document = await createOrgDocumentLink(orgId, {
        title,
        categoryId: categoryDraft,
        url,
        purpose: purposeDraft || null,
    ...
    <>
      ...
      {addMode === 'upload' ? (
        <Modal
          ...
          layer="overlay"
          devComponentName="OrgDocumentAddModal"
        >
          <div
            className="space-y-4"
            onDragEnter={(event) => {
      ...
      {addMode === 'link' ? (
        <Modal
          ...
          layer="overlay"
          devComponentName="OrgDocumentLinkModal"
        >
