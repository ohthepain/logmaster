...
export function BoatAssetsTab({
  ...
  const [uploadingDocForAssetId, setUploadingDocForAssetId] = useState<
    string | null
  >(null)
  ...
  const loadWork = useCallback(
    async (assetId: string) => {
      try {
        ...
        toast.error(
          e instanceof Error ? e.message : 'Failed to load work records',
        )
      }
    },
    [boatId],
                    ...
                    <div className="mt-4">
                      ...
                      <AssetDocumentsSection
                        ...
                        onUpload={(file, purpose) =>
                          void handleAssetDocumentUpload(
                            asset.id,
                            file,
                            purpose,
                          )
          ...
          onSave={async (input) => {
            ...
            try {
              if (assetModal.mode === 'edit') {
                const updated = await updateBoatAsset(
                  boatId,
                  assetModal.asset.id,
                  input,
                )
      ...
      {workModal.mode === 'create' ? (
        <WorkModal
          ...
          onSave={async (input) => {
            setBusy(true)
            try {
              const work = await createAssetWork(
                boatId,
                workModal.asset.id,
                input,
              )
