...
async function entryMarkersFromGeoJson(
  ...
  for (const feature of collection.features) {
    ...
    const entryId =
      typeof feature.properties?.entryId === 'string'
    ...
    const color =
      typeof feature.properties?.color === 'string'
        ...
        : null
    const outline = feature.properties?.outline as
      | LogEntryMapOutline
      | undefined
...
function trackCoordinatesFromGeoJson(
  collection: GeoJSON.FeatureCollection,
): MapCoordinate[] {
...
export const TripAppleMapKit = forwardRef<
  TripAppleMapKitHandle,
  TripAppleMapKitProps
>(function TripAppleMapKitView(
  {
    trip,
    entries,
    legs = [],
  ...
  const currentPositionRef = useRef<
    (MapCoordinate & { heading?: number | null }) | null
  >(null)
  ...
  const previewEntry = entryPreview
    ? (entriesById.get(entryPreview.entryId) ?? null)
    : null
  ...
  const syncBoatMarker = useCallback(async () => {
    ...
    const livePosition =
      showCurrentPosition && !playbackPosition
  ...
  return (
    <div className="relative h-full min-h-0 w-full overflow-hidden bg-transparent">
      {!embedded ? (
        <DevComponentLabel
          name="TripAppleMapKit"
          className="absolute left-2 top-2 z-10"
        />
      ) : null}
      <div
        className={cn('pointer-events-none bg-transparent', mapClassName)}
        aria-hidden
      />
