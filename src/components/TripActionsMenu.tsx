...
export function TripActionsMenu({
  ...
  const showRetripAction = devMode && isDevModeAvailable() && trip.status === "COMPLETED";
  ...
  const handleExportSignalK = () => {
    void runAction(async () => {
      ...
      const hydratedTracks = useLogbookStore.getState().tracks.filter((track) => track.tripId === trip.id);
      const tripEntries = entries.filter((entry) => entry.tripId === trip.id && !entry.deleted);
        ...
        >
          <div className="space-y-4">
            <p className="m-0 text-sm leading-6 text-[var(--sea-ink-soft)]">
              ...
              {entryCount > 0 ? ` and all ${entryCount} log ${entryCount === 1 ? "entry" : "entries"}` : ""}? This
              cannot be undone.
            </p>
