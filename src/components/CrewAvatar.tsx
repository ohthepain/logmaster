...
export function CrewAvatar({ name, imageUrl, imagePath, userId, className }: CrewAvatarProps) {
  const [imageFailed, setImageFailed] = useState(false);

  const src = imageUrl != null ? imageUrl : userId && imagePath ? crewUserPhotoUrl(userId, imagePath) : null;
  ...
  return (
    ...
    >
      {showPhoto ? (
        <img src={src!} alt="" className="size-full object-cover" loading="lazy" onError={() => setImageFailed(true)} />
      ) : (
