...
export const TripImportButton = forwardRef<
...
>(function TripImportButtonView(
  ...
  const menuId = useId()
  const importTripFromGpxFiles = useLogbookStore(
    (state) => state.importTripFromGpxFiles,
  )
  const importTripFromSignalK = useLogbookStore(
    (state) => state.importTripFromSignalK,
  )
  const importRoutesFromGpxFiles = useRoutesStore(
    (state) => state.importRoutesFromGpxFiles,
  )
  ...
  const importFromGpxFiles = async (files: File[]) => {
    ...
    if (tripFiles.length === 0 && routeFiles.length === 0) {
      throw new GpxImportError(
        'No GPX track or route data was found in that selection.',
      )
    }
  ...
  const handleGpxSelection = async (
    ...
    try {
      ...
      if (
        openFolderPickerOnFailure &&
        error instanceof GpxFolderImportNeededError
      ) {
  ...
  const handleSignalKFileChange = async (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const selectedFiles = event.target.files ? [...event.target.files] : []
    event.target.value = ''
