...
import { identifyAsset, identifyAssetFromLink, normalizeAssetPhoto, researchAsset } from "./asset-intelligence";
...
describe("TanStack AI asset service", () => {
  it("uses web search to identify equipment from a product link", async () => {
    ...
    await identifyAssetFromLink("https://example.com/garmin-923");
    expect(mocks.web).toHaveBeenCalled();
  ...
  it("sends photo bytes through the TanStack multimodal adapter with structured output", async () => {
    ...
    expect(mocks.adapter).toHaveBeenCalledWith("gpt-5.4-mini");
    expect(mocks.chat).toHaveBeenCalledWith(
      expect.objectContaining({
        outputSchema: expect.anything(),
        messages: [
  ...
  it("requires web search and rejects unknown connection IDs and duplicate documents", async () => {
    ...
    expect(result.connections).toEqual([{ assetId: "tank", reason: "Water supply" }]);
    expect(mocks.chat).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("normalizes photo pixels and strips metadata before storage or AI use", async () => {
    const source = await sharp({
      ...
      .withMetadata()
      .toBuffer();
    const photo = await normalizeAssetPhoto(new File([new Uint8Array(source)], "photo.jpg", { type: "image/jpeg" }));
  ...
  it("rejects files that claim to be images but cannot be decoded", async () => {
    await expect(normalizeAssetPhoto(new File(["<html>"], "photo.jpg", { type: "image/jpeg" }))).rejects.toThrow(
      "could not be read",
    );
  });
});
