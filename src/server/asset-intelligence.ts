...
import { identificationSchema, researchSchema } from "./asset-intelligence-schema";
...
function assetAdapter() {
  if (!process.env.OPENAI_API_KEY)
    throw new Error("AI identification is not configured. You can still add the asset manually.");
...
export async function normalizeAssetPhoto(file: File) {
  ...
  try {
    ...
    throw new Error("This photo could not be read. Please choose a JPEG, PNG or WebP photo.");
  }
}

async function withTimeout<T>(run: (abortController: AbortController) => Promise<T>): Promise<T> {
  ...
  return withAiResponseLog("asset.research", async () => {
    ...
    return {
      ...
      connections: result.connections.filter(
        (item, index, all) => ids.has(item.assetId) && all.findIndex((other) => other.assetId === item.assetId) === index,
      ),
    };
  });
}
