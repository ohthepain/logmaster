...
export const MAP_DATA_LAYERS: MapDataLayerDefinition[] = [
  ...
  },
  {
    ...
    title: 'Spot depths',
    description:
      'Tappable OSM depth soundings — sparse; rebuild seamarks after query updates.',
    group: 'bathymetry',
    defaultVisible: false,
    persistToggle: true,
    dataset: 'seamarks',
  ...
  },
  {
    ...
    description:
      'Current ship positions in the map view — coverage varies vs commercial charts (internet only).',
    group: 'navigation',
    defaultVisible: true,
    persistToggle: true,
    liveOnly: true,
...
export function defaultMapDataLayerToggles(): MapDataLayerToggles {
  return Object.fromEntries(
    MAP_DATA_LAYERS.map((layer) => [layer.id, layer.defaultVisible]),
...
export function mapDataLayerAuxiliaryLayerId(
  layerId: MapDataLayerId,
): string | null {
  if (layerId === 'osm-depth-soundings') {
    return mapDataLayerSymbolLayerId(layerId)
