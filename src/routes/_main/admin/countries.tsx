    ...
    <main className="page-wrap px-4 py-8">
      <section className="island-shell rounded-2xl p-6 sm:p-8">
        ...
        <p className="m-0 mb-4 max-w-2xl text-sm leading-6 text-[var(--sea-ink-soft)]">
          Import GeoNames-style <code className="text-xs">countryInfo.txt</code>{' '}
          (tab-separated, <code className="text-xs">#</code> comments). Header
          row{' '}
          <code className="text-xs">
            #ISO · ISO3 · ISO-Numeric · … · Languages · …
          </code>{' '}
          is skipped. Commit the JSON as{' '}
          <code className="rounded bg-[var(--chip-bg)] px-1.5 py-0.5 text-xs">
            public/data/countries.json
          </code>{' '}
          (served at <code className="text-xs">/data/countries.json</code>).
          Cached in IndexedDB after the first successful fetch.
        </p>
          ...
          <table className="w-full border-collapse text-left text-sm">
            <thead className="sticky top-0 bg-[var(--header-bg)]">
              <tr className="border-b border-[var(--line)]">
                <th className="px-2 py-2 font-medium text-[var(--sea-ink)]">
                  Code
                </th>
                <th className="px-2 py-2 font-medium text-[var(--sea-ink)]">
                  Name
            ...
            <tbody>
              {shown.map((c) => (
                ...
                >
                  ...
                  <td className="px-2 py-1.5 text-[var(--sea-ink-soft)]">
                    {c.name}
                  </td>
                  <td className="px-2 py-1.5 text-[var(--sea-ink-soft)]">
                    {c.capital || '—'}
