...
The composer waits 550 ms after typing a phrase (up to 200 characters). Matching requires the entire saved phrase, ignoring case, surrounding whitespace and repeated whitespace only. Punctuation and character width remain significant. Exact matches in the selected language are followed by exact English matches, deduplicated. Automatic translation is no longer used for suggestions: it could correct a fragment or typo into a matching phrase. Stale suggestions disappear immediately when the text changes, including backspace.

Tapping a suggestion sends the card immediately without text or attachments and clears the input. Failed sends restore the draft unless the user has already typed a new one; retries reuse the message ID. Existing attachment drafts remain for a separate send. Cards display with transparent backgrounds and no user text. Adjacent cards from the same sender and thread are arranged left to right, wrapping when necessary, if each gap is less than one minute. Any other message or a gap of one minute ends the group; likes remain per message.

Apply `20260925100000_exact_card_expressions` before deploying these matching changes. It regenerates normalized phrases from their original saved text, preserving punctuation.

The server resolves only active card IDs and writes a provider-neutral, versioned snapshot into `ChatMessage.responseCard`. Arbitrary client card JSON or asset URLs are rejected. Stream remains only a realtime invalidation transport, with the existing app notification pipeline.

Deployment: apply migration `20260921210000_response_cards` and Terraform's ECS environment/task-role changes. The existing `logmaster-messaging-cards` bucket is private and AES256 encrypted. Bucket resources are optional (`manage_messaging_cards_bucket`); leave false for this existing externally managed bucket, or import it before enabling management. Other environments should set their own bucket name. Response cards do not require an OpenAI key. This change does not itself deploy the app or apply production database migrations.
