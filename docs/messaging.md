...
5. Run the existing `pnpm worker` process. The message and push outboxes recover at least once a minute, with immediate queue wake-ups for new messages. Check `chat_message.publishedAt` backlog and `push_dispatch.failedAt` for failures.
